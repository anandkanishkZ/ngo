...
@php
  $relatedNotices = \App\Models\Notice::published()
      ...
      ->when(!$notice->category, function($query) use ($notice) {
          return $query->where('priority', $notice->priority);
      })
      ->latest()
      ->limit(3)
      ->get();
...
<section class="related-notices-section py-5 bg-light">
  <div class="container">
    <div class="section-header text-center mb-5">
      <h3 class="fw-bold">Related Notices</h3>
      <p class="text-muted">Other important notices you might be interested in</p>
    </div>
    <div class="row">
      @foreach($relatedNotices as $related)
        <div class="col-lg-4 col-md-6 mb-4">
          <div class="related-notice-card h-100">
            <div class="card-image-wrapper">
              <img src="{{ $related->image ? asset('storage/notices/' . $related->image) : 'https://images.unsplash.com/photo-1560472354-b33ff0c44a43?q=80&w=400&auto=format&fit=crop' }}" 
                   class="card-img-top" 
                   alt="{{ $related->title }}"
                   style="height: 200px; object-fit: cover;">
              <div class="card-overlay">
                <span class="priority-badge priority-{{ $related->priority }}">
            ...
            <div class="card-body d-flex flex-column">
              <div class="card-meta mb-2">
                ...
                @if($related->category)
                  <span class="badge bg-light text-dark ms-2">{{ $related->category }}</span>
                @endif
              </div>
              <h5 class="card-title">{{ $related->title }}</h5>
              <p class="card-text text-muted flex-grow-1">{{ $related->excerpt }}</p>
              <a href="{{ route('notices.show', $related) }}" class="btn btn-outline-primary btn-sm">
