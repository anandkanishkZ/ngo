...
class HeroSlideController extends Controller
{
    ...
    public function update(Request $request, HeroSlide $slide)
    {
        ...
        if ($request->hasFile('bg_image')) {
            ...
            $oldFile = basename($slide->bg_image);
            if ($oldFile && file_exists(public_path('uploads/hero/' . $oldFile))) {
                unlink(public_path('uploads/hero/' . $oldFile));
            }
            $file = $request->file('bg_image');
            $filename = time() . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
            $file->move(public_path('uploads/hero'), $filename);
    ...
    {
        $filename = basename($slide->bg_image);
        if ($filename && file_exists(public_path('uploads/hero/' . $filename))) {
            unlink(public_path('uploads/hero/' . $filename));
        }
        $slide->delete();
