...
class MediaController extends Controller
{
    ...
    private function processUpload($file, $folder = null)
    {
        ...
        \Log::info('File info - Size: ' . $size . ', MIME: ' . $mimeType . ', IsImage: ' . (str_starts_with($mimeType, 'image/') ? 'yes' : 'no'));
        \Log::info('Original filename: ' . $originalName . ', Extension: ' . $extension);
        ...
        \Log::info('Creating media record with data:', $mediaData);

        try {
            // Create media record
            $media = Media::create($mediaData);
            \Log::info('Media record created successfully with ID: ' . $media->id);
            return $media;
        } catch (\Exception $e) {
