...
function initScrollEffects() {
    ...
    const scrollThresholdDown = 60; // Threshold for adding 'scrolled' class
    const scrollThresholdUp = 30;   // Threshold for removing 'scrolled' class (hysteresis)
    let isScrolled = false; // Track current state to prevent rapid toggling
    ...
    function updateStickyHeader() {
        ...
        if (!isScrolled && scrollTop > scrollThresholdDown) {
            // Debounce the addition of scrolled class
            debounceTimer = setTimeout(() => {
                header.classList.add('scrolled');
                isScrolled = true;
            }, 50);
        } else if (isScrolled && scrollTop < scrollThresholdUp) {
            // Debounce the removal of scrolled class
            debounceTimer = setTimeout(() => {
    ...
    if (header) {
        // Remove conflicting inline styles that might cause jerking
